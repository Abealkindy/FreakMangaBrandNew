...
public class NhenFavouriteFragment extends Fragment implements SearchView.OnQueryTextListener {
    private FragmentNhenLocalBinding mBinding;
    private Context mContext;

    public NhenFavouriteFragment() {
    ...
    private void getDataFromLocalDB(String hitStatus, String newText) {
        ...
        if (hitStatus.equalsIgnoreCase("ordinary")) {
            ...
            historyModelList = localAppDB.nhenBookmarkDAO().searchByName("%" + newText + "%");
            if (validateList(historyModelList)) {
                showRecyclerResult(historyModelList);
            } else {
    ...
    private boolean validateList(List<NhenBookmarkTable> historyModelList) {
        return historyModelList != null && historyModelList.size() > 0;
    }

    private void showRecyclerResult(List<NhenBookmarkTable> historyModelList) {
        ...
        mBinding.recylerAnimeBookmark.setAdapter(new MainMangaAdapter(requireActivity(), historyModelList, false));
